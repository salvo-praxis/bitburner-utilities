...
export async function main(ns: NS) {
    ...
    function biosHeader() {
        ...
        ns.tprint(
            c.yellow + "→ deploy-scripts.js: DEPLOY SCRIPTS (REMOTE) ─ DISTRIBUTED HGW" + c.reset
        );
        ns.tprint(
            c.cyan + "• Deploys optimized HGW scripts to all rooted servers and targets." + c.reset
        );
    }

    function showHelp() {
        ns.tprint(
            `${c.bold + c.green}deploy-scripts.js${c.reset} ${c.white}| Distributed script deployment${c.reset}\n\n` +
    ...
    const capableServers: CapableServer[] = rootedServers
        .filter(h => h !== "home")
    ...
    let threadAssignments: { [key: string]: { [key: string]: number } } = {};
        ...
        for (const tgt of validTargets) {
            ...
            for (let i = 0; i < maxRetries; i++) {
                if (ns.exec(path, srv.name, th, tgt)) {
                    biosMsg(
                        "SUCCESS",
                        `${srv.name}: Deployed ${path}${c.white} ${colorBySize(th)}x${th}${c.white} ${arrowWhite} ${c.white}${tgt}`
                    );
                    totalDeployed += th;
                    deployedCount++;
            ...
            if (!ok) biosMsg("TERMINATE", `${srv.name}: Failed to exec ${path} on ${tgt}`);
        }
        if (executionDelay > 0) await ns.sleep(executionDelay); // only delay if > 0
    }
