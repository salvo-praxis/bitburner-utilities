...
export async function main(ns) {
    ...
    if (args.help) {
        ns.tprint(
            color.cyan + "pserv.js -- Bitburner Personal Server Utility\n" + color.reset +
            "\n" +
            color.yellow + "Usage:\n" + color.reset +
    ...
    if (args.mode === "list") {
        ...
        for (let i = 0; i < options.length; ++i) {
            ...
            if (rawPct > bestPct) { bestPct = rawPct; bestIdx = i; }
        }
        ...
        for (let i = 0; i < options.length; ++i) {
            ...
            let ramAmt = r * 25; // GB
            let ramStr =
                ramAmt >= 1048576 ? color.cyan + (ramAmt / 1048576).toFixed(2) + " PB" + color.reset :
                ramAmt >= 1024    ? color.cyan + (ramAmt / 1024).toFixed(2) + " TB" + color.reset :
                                    color.cyan + ramAmt + " GB" + color.reset;

            out.push(
                pad(color.yellow + (i + 1) + color.reset, idxW, true) + " " +
    ...
    if (args.mode === "status") {
        ...
        if (owned.length === 0) out.push(color.gray + "You don't own any servers." + color.reset);
        else {
            for (let s of owned) {
                let ram   = ns.getServerMaxRam(s);
                let used  = ns.getServerUsedRam(s);
                let procs = ns.ps(s);
                out.push(
                    color.green + pad(s, 22) + color.reset +
                    pad(ramLabel(ram), 8) +
    ...
    if (args.mode === "buy") {
        ...
        if (isNaN(count) || count <= 0) return ns.tprint(color.red + `Invalid count: ${args.count}` + color.reset);
        ...
        if (slotsLeft <= 0) return ns.tprint(color.red + `ERROR: Already own ${owned.length}/${limit} servers.` + color.reset);
        if (count > slotsLeft) {
        ...
        if (bought === 0) out.push(color.red + "No servers purchased." + color.reset);
        ns.tprint("\n" + out.join("\n"));
        return;
    }
