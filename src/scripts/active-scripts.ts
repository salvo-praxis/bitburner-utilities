...
export async function main(ns) {
    ...
    function ramBar(percent, width = 30) {
        ...
        if (percent === 0) fillColor = color.white;
        else if (percent < 40) fillColor = color.red;
        else if (percent < 80) fillColor = color.yellow;
    ...
    function colorPercent(p) {
        if (p === 0) return color.gray + `${p.toFixed(1)}%` + color.reset;
        if (p < 40) return color.red + `${p.toFixed(1)}%` + color.reset;
        if (p < 80) return color.yellow + `${p.toFixed(1)}%` + color.reset;
    ...
    function colorServerName(name, percent) {
        ...
        let colorize;
        if (percent === 0 || percent >= 100) colorize = color.white;
        else if (percent < 40) colorize = color.red;
        else if (percent < 80) colorize = color.yellow;
    ...
    function highlightFilename(path, width = 28, percent = 100) {
        ...
        if (percent < 40) colorize = color.red;
        else if (percent < 80) colorize = color.yellow;

        return pad(
            color.red + dir + colorize + nameOnly + color.red + "." + colorize + "js" + color.reset,
            width
        );
    }
    ...
    if (hostname === "all") {
        ...
        for (const entry of data) {
            buffer.push(
                pad(colorServerName(entry.name.slice(0, 20), entry.percent), 20) +
    ...
    for (const proc of data) {
        ...
        buffer.push(
            highlightFilename(proc.filename, 28, percent) +
