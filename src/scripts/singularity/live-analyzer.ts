/**
 * live-analyzer.ts (v1.1 - Singularity Optimized)
 * Real-time analyzer for currently running scripts.
 * Requires Singularity API for script text inspection.
 *
 * CLI Flags:
...
export async function main(ns) {
    ...
    for (const p of processes) {
        ...
        if (ns.fileExists(p.filename, server)) {
            const content = ns.read(p.filename);
            const functions = (content.match(/ns\.[\w\.]+/g) || []).filter(fn => !fn.includes("//"));
            const usage = {};
            for (const fn of functions) {
                const stripped = fn.replace(/^ns\./, "");
                try {
                    const cost = ns.getFunctionRamCost(stripped) || 0;
                    usage[fn] = (usage[fn] || 0) + cost;
                } catch {}
            }

            const dedupedTotal = Object.values(usage).reduce((a, b) => a + b, 0);

            outputBuffer.push(`  ${color.gray}Detected NS calls: ${Object.keys(usage).length}, Est. NS RAM: ${dedupedTotal.toFixed(2)} GB${color.reset}`);
        } else {
            outputBuffer.push(`  ${color.gray}Script not found on ${server}, skipping analysis.${color.reset}`);
        }
    }
