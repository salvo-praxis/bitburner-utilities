        ...
        for (let f of factions) {
            ...
            if (ns.getPlayer().money >= price && factionRep >= rep) {
                if (!args["dry-run"]) {
                    ...
                    if (!ok) continue; // Defensive: only log if actually bought
                }
                purchases.push({faction: f, nfgNum: n + 1, price, rep});
                if (!factionPurchases[f]) factionPurchases[f] = [];
                ...
                ns.tprint(
                    `${c.red}[${c.bold}${c.white}neuroflux.js${c.reset}${c.red}]${c.reset} ` +
                ...
                if (!args["dry-run"]) await ns.sleep(80); // Yield for UI stability
            }
        }
        if (!boughtThisRound) break; // None purchased in this round, stop.
    }
