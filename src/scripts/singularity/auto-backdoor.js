...
export async function main(ns) {
    ...
    function flushLogs() {
        if (args.log) ns.write(logFile, logBuffer.join("\n") + "\n", "w");
        logBuffer = [];
    }
    ...
    if (args.help || (!args.server && !args.all && !args.update)) {
        let helpBuf = [];
        helpBuf.push(""); helpBuf.push(""); // Leading space
        ...
        if (ns.singularity && ns.singularity.connect) ns.singularity.connect("home");
        return;
    }
    ...
    if (args.update) {
        ...
        while (queue.length) {
            ...
            for (const n of ns.scan(cur)) if (!all.has(n)) { all.add(n); queue.push(n); }
        }
        const backdoored = [...all].filter(s =>
            s !== "home" &&
            ns.hasRootAccess(s) &&
            ns.getServer(s).backdoorInstalled
        );
        log(
            color.cyan + `Updated ${color.yellow}${backdoored.length}${color.cyan} backdoored servers in ${color.yellow}${backdooredServersFile}` + color.reset
        );
    ...
    const cachedScan = host => {
        if (!scanCache.has(host)) scanCache.set(host, ns.scan(host));
    ...
    if (args.all) {
        ...
        do {
            ...
            while (queue.length) {
                const cur = queue.shift();
                for (const n of ns.scan(cur)) if (!allServers.has(n)) { allServers.add(n); queue.push(n); }
            }
            ...
            if (targets.length === 0) {
                if (persistent) {
                    log(
                        color.yellow + "💤 No viable servers found, sleeping for " +
                        color.cyan + (interval/1000).toFixed(1) + "s..." + color.reset
                    );
                } else {
                    log(
                        color.cyan + "Starting auto-backdoor: " +
