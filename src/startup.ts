...
export async function main(ns) {
    ...
    if (args.force) {
        ...
        if (!forceTarget) return ns.tprint(c.red + "[startup.js]" + c.reset + " Must specify script basename for --force.");
        let scriptList = findScriptsByBasename(ns, forceTarget);
        if (!scriptList.length) return ns.tprint(c.red + "[startup.js]" + c.reset + ` No matching scripts for --force: ${forceTarget}`);
    ...
    if (args.add) {
        ...
        if (!script) return ns.tprint(c.red + "[startup.js]" + c.reset + " Must specify script path for --add.");
        ...
        if (!addParams.some(x => x.startsWith("--delay="))) line += ` --delay=${delay}`;
    ...
    if (args.update !== false) {
        ...
        if (!target) return ns.tprint(c.red + "[startup.js]" + c.reset + " Must specify script basename (or line #) for --update.");
    ...
    if (args.remove !== false) {
        ...
        if (!target) return ns.tprint(c.red + "[startup.js]" + c.reset + " Must specify script basename (or line #) for --remove.");
...
function biosMsg(ns, c, script, type, msg, color, args, updateOnly = false) {
    if (args.silent) return;
    if (args.quiet && ["UPDATE", "NOTICE"].includes(type)) return;
    let now = timestamp();
...
function getParamsAfterFlag(ns, args, flag) {
    ...
    for (let i = idx + 2; i < process.argv.length; ++i) {
        if (process.argv[i].startsWith("--") && arr.length) break;
