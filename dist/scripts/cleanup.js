...
export async function main(ns) {
    ...
    const remotes = servers.filter(s => s.name && s.name !== "home" && !s.name.startsWith("pserv-") && ns.getServerMaxRam(s.name) > 0);
    let targetSets = [];
    if (args.home)
        targetSets.push({ label: "home", servers: [home] });
    if (args.remote || (!args.home && !args.purchased))
        targetSets.push({ label: "remote", servers: remotes });
    if (args.purchased)
        targetSets.push({ label: "purchased", servers: purchased });
...
function biosHeader(ns, c, args) {
    if (args.silent)
        return;
    ns.tprint(c.yellow + "→ cleanup.js: CLEANUP ─ MODULAR UTILITY" + c.reset);
    ns.tprint(c.cyan + "→ Automated server process & file cleanup tool." + c.reset);
}
function showHelp(ns, c) {
    ns.tprint(`${c.bold + c.green}cleanup.js${c.reset} ${c.white}| Modular server cleanup utility${c.reset}\n\n` +
