...
export async function main(ns) {
    ...
    while (true) {
        ...
        for (const server of servers) {
            ...
            if (availableOpeners.length < reqPorts)
                continue;
            ...
            for (let i = 0; i < reqPorts; ++i) {
                try {
                    if (!args["dry-run"])
                        availableOpeners[i].fn(ns)(server);
                }
                catch (_) { }
            }
            // Nuke
            ...
            if (ns.hasRootAccess(server) && !nuked.has(server)) {
                ...
                if (!args["dry-run"])
                    await ns.write(OUTPUT_FILE, server + "\n", "a");
            }
            await ns.sleep(100);
        }
...
function biosHeader(ns, c, args) {
    if (args.silent)
        return;
    ns.tprint(c.yellow + "→ brute-nuke.js: BRUTE-NUKE ─ NETWORK ROOTER" + c.reset);
    ns.tprint(c.cyan + "→ Automated and optionally persistent brute-nuking tool." + c.reset);
}
function showHelp(ns, c) {
    ns.tprint(`${c.bold + c.green}brute-nuke.js${c.reset} ${c.white}| Modular network nuke automation${c.reset}\n\n` +
