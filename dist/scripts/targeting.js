...
export async function main(ns) {
    ...
    function colorScore(score, min, max, width) {
        if (isNaN(score))
            return color.white + fit("-", width) + color.reset;
        let ratio = (score - min) / Math.max(max - min, 1);
        let raw = fit(score.toFixed(1), width);
        if (ratio < 0.2)
            return color.red + raw + color.reset;
        if (ratio < 0.5)
            return color.yellow + raw + color.reset;
        if (ratio < 0.8)
...
- Output is colorized and aligned in BIOS-format for PRAAXIS_ALPHA.
    ...
    try {
        ...
        for (const line of raw) {
            ...
            if (host && ns.serverExists(host.trim()))
                servers.push(host.trim());
        }
    }
    catch (e) {
    ...
    for (const r of lines) {
        ...
        outBuffer.push(hostCol + " " + scoreCol + moneyCol + chanceCol + timeCol + levelCol);
    }
    if (args.write) {
