...
export async function main(ns) {
    ...
    if (args.help)
        return ns.tprint("Usage: run aug-priority.js --mode hacking --top 10");
    // Color codes
    const color = {
        reset: "\u001b[0m",
        green: "\u001b[32m",
    ...
    if (next)
        planLines.push(color.yellow + `Then grind rep for #${augList.indexOf(next) + 1} (${next.name}) at ${next.bestFaction}.` + color.reset);
    ...
    for (const far of augList.filter(a => a.statusRaw === "FAR")) {
        ...
        planLines.push(color.red + `#${augList.indexOf(far) + 1} (${far.name})` + color.reset +
...
function getShortType(stats, mode) {
    let key = Object.entries(stats)
        ...
        .filter(([k, v]) => {
        if (mode === "hacking")
            return k.includes("hack");
        if (mode === "crime")
            return k.includes("crime");
        if (mode === "company")
            return k.includes("company");
        if (mode === "faction")
            return k.includes("faction");
        return true;
    })
...
function buildChain(ns, chain, owned, augSources, joinedFactionReps, money, depth = 1) {
    ...
    for (const aug of chain) {
        if (owned.includes(aug))
            continue; // skip already-owned prereqs
