...
export async function main(ns) {
    ...
    function augColor(aug) {
        if (/NeuroFlux/.test(aug))
            return color.red;
        if (/Daedalus|Red Pill/.test(aug))
            return color.magenta;
        if (/Cranial|Neural|Signal/.test(aug))
            return color.cyan;
        if (/gang|crime|bladeburner/i.test(aug))
            return color.yellow;
        return color.white;
    }
    let augCounts = {};
    for (const a of augs)
        augCounts[a] = (augCounts[a] || 0) + 1;
    // Source-Files
    ...
    // Factions/companies
    let factions = player.factions.map(f => color.yellow + f + color.reset +
        (ns.singularity.getFactionRep(f) > 0 ? color.green + ` (${ns.nFormat(ns.singularity.getFactionRep(f), "0.0a")} rep)` + color.reset : "")).join(", ");
    let companies = Object.keys(player.jobs)
        .map(c => `${color.cyan}${c}${color.reset} (${player.jobs[c]}, ${color.gray}${ns.nFormat(ns.singularity.getCompanyRep(c), "0.0a")} rep${color.reset})`).join(", ");
    // Gang
    ...
    // 1. Header / Position
    let out = [];
    out.push(`${color.cyan}${pname}${color.reset}   ` +
    ...
    out.push(" " + augs.map(a => augColor(a) + a + (augCounts[a] > 1 && a === "NeuroFlux Governor" ? ` x${augCounts[a]}` : "") + color.reset).join(", "));
    out.push("");
    // 4. Source-Files
    ...
    if (companies)
        out.push(`Companies: ${companies}`);
    out.push("");
    // 6. Gang (if applicable)
    if (gang) {
        out.push(`Gang: ${color.cyan}${gang.faction}${color.reset}  Power: ${color.green}${ns.nFormat(gang.power, "0.00a")}${color.reset}  ` +
    ...
    if (notoriety > 0)
        out.push(`Notoriety: ${color.magenta}${notoriety}${color.reset}`);
    out.push("");
    // 8. Tips (expanded for demo)
    out.push("Tips:");
    if (karma <= -54000 && !gang)
        out.push(color.green + "Eligible to create a gang!" + color.reset);
    if (augs.includes("NeuroFlux Governor"))
        out.push(color.yellow + `You have purchased NeuroFlux Governor x${nfgCount}.` + color.reset);
    ...
    for (let f of player.factions) {
        ...
        for (let a of fa) {
            ...
            if (rep >= r && !augsAvail.includes(a))
                augsAvail.push(a);
        }
    }
    if (augsAvail.length > 0)
        out.push(color.green + `You can purchase new augmentations: ${augsAvail.join(", ")}` + color.reset);
    ns.tprint("\n" + out.join("\n"));
}
