...
export async function main(ns) {
    ...
    try {
        corpUnlocked = ns.corporation.hasCorporation();
    }
    catch { }
    try {
        gangUnlocked = ns.gang.inGang();
    }
    catch { }
    try {
        bladeUnlocked = !!ns.bladeburner.getRank();
    }
    catch { }
    // ---- PHASE DETECTION ----
    let phase = "Early";
    if (corpUnlocked || gangUnlocked || bladeUnlocked || ownedAugs.length >= 1 || hacking >= 500)
        phase = "Mid";
    if (ownedAugs.length >= 10 || (corpUnlocked && ns.corporation.getCorporation().funds > 1e12) || hacking >= 1500)
        phase = "Late";
    // ---- HACKING TARGETS ----
    ...
    try {
        ...
        if (bestAug)
            augAdvice = `You can afford '${bestAug}' if you reach ${ns.nFormat(repNeeded, "0.0a")} rep in a joined faction.`;
    }
    catch (e) {
    ...
    let actions = [];
    if (phase === "Early")
        actions.push("Focus: unlock Corp/Gang, build Home RAM, buy all hack programs, backdoor servers for factions.");
    if (phase === "Mid")
        actions.push("Focus: buy augs, push corp/gang/Bladeburner growth, upgrade purchased servers, target key factions.");
    if (phase === "Late")
        actions.push("Focus: buy last augs, NeuroFlux grind, consider prestige/install augs.");
    // Major recommendations, prioritized
    if (!hasTor && money < 200000)
        actions.push(`Earn $${ns.nFormat(200000 - money, "0.0a")} for Tor router.`);
    else if (!hasTor)
        actions.push("Buy Tor router from terminal for $200k.");
    else if (missingPrograms.length > 0)
        actions.push(`Buy missing hack programs: ${missingPrograms.join(", ")}.`);
    if (hacking < 50)
        actions.push(`Level up hacking to 50+ for CSEC backdoor (now: ${hacking}).`);
    else if (hacking < 100)
        actions.push(`Raise hacking to 100+ for more servers (now: ${hacking}).`);
    if (homeRam < 32 && money > 1e6)
        actions.push(`Upgrade 'home' RAM (now ${homeRam}GB).`);
    if (!corpUnlocked && money >= 150e9)
        actions.push(`$${ns.nFormat(money, "0.0a")} (>$150B): Start Corporation for major passive income.`);
    else if (!corpUnlocked && money >= 150e6)
        actions.push(`$${ns.nFormat(money, "0.0a")} (>$150M): Start Corporation (slower, but valid).`);
    if (!gangUnlocked && hacking >= 100 && karma > -54000)
        actions.push(`Homicide crime to karma -54,000 (now: ${karma.toFixed(1)}, ${gangPct.toFixed(1)}%).`);
    else if (!gangUnlocked && karma <= -54000)
        actions.push(`Eligible for Gang (karma: ${karma.toFixed(1)}). Join in Slum City.`);
    if (!bladeUnlocked && allCombat100 && hacking >= 250)
        actions.push("You can join Bladeburner (combat 100+, hacking 250+).");
    ...
    if (backdoorSuggestions.length > 0)
        actions.push(...backdoorSuggestions);
    if (augAdvice)
        actions.push(augAdvice);
    if (ownedAugs.length > 2 && money > 1e9)
        actions.push("Buy more augs from factions with enough rep/cash.");
    if (ownedAugs.length > 10)
        actions.push("10+ augs owned: consider installing/prestige soon.");
    if (!hacknetUnlocked)
        actions.push("Purchase a Hacknet node for passive income.");
    if (psAdvice)
        actions.push(psAdvice);
    // ---- OUTPUT ----
    ns.tprint(`
