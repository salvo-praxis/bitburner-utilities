...
export async function main(ns) {
    ...
    function biosHeader() {
        if (args.silent)
            return;
        ns.tprint(c.yellow + "→ deploy-home.js: DEPLOY HOME ─ DISTRIBUTED HGW" + c.reset);
        ns.tprint(c.cyan + "→ Deploys optimized HGW scripts on home to all distributed targets." + c.reset);
    }
    function showHelp() {
        ns.tprint(`${c.bold + c.green}deploy-home.js${c.reset} ${c.white}| Home script deployment${c.reset}\n\n` +
    ...
    biosMsg("INIT", `Usable RAM: ${colorBySize(usableRam)}${usableRam.toFixed(2)}GB${c.white} (${args["max-ram"]}% ${args["killall"] ? "of total" : "of available"}), using ${script} (${colorBySize(scriptRam)}${scriptRam.toFixed(2)}GB/thread${c.white})`);
    // Thread assignment
    ...
    for (const tgt of targets) {
        ...
        for (let i = 0; i < maxRetries; i++) {
            if (ns.exec(script, HOME, th, tgt)) {
                biosMsg("SUCCESS", `Deployed ${scriptName}${c.white} ${colorBySize(th)}x${th}${c.white} ${c.cyan}→${c.white} ${tgt}`);
                launchedThreads += th;
        ...
        if (!ok)
            biosMsg("TERMINATE", `🛑 Failed to deploy ${scriptName} x${th} -> ${tgt} after ${maxRetries} retries`);
    }
