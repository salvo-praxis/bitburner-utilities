...
export async function main(ns) {
    ...
    function groupAPI(fn) {
        if (fn.startsWith("ns.corporation.")) return "corporation";
        if (fn.startsWith("ns.bladeburner.")) return "bladeburner";
        if (fn.startsWith("ns.gang.")) return "gang";
        if (fn.startsWith("ns.singularity.")) return "singularity";
        if (fn.startsWith("ns.hacknet.")) return "hacknet";
        if (fn.startsWith("ns.heart.")) return "heart";
        return "core";
    }

    function getRamCost(fn) {
        if (!fn.startsWith("ns.")) return 0;
